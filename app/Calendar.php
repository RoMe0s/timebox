...
class Calendar extends Model
{
	...
	protected $fillable = ['id', 'color', 'font_color', 'description', 'time_from',
		'time_to', 'employee_id', 'service_id', 'client_id', 'group_id',
		'domain', 'date', 'status', 'date_deleted', 'site', 'sms', 'email', 'is_empl_email', 'is_client_email'];
	...
	public function getCalendar($nameDomain, $dates, $id, $cart_id = 0)
	{
		if (Auth::user()->status === 'admin' || Auth::user()->status === 'admin_employee') {
			$calendar = self::where('domain', $nameDomain)
				...
				->leftJoin('services_category', 'services_category.id', '=', 'services.category_id')
				->select('calendar.id', 'calendar.color', 'calendar.font_color', 'calendar.description', 'calendar.time_from',
					...
					'calendar.service_id', 'services.service_name','services.price', 'calendar.group_id', 'calendar.email as send_email',
					'calendar.sms as send_sms', 'services.duration','services_category.category_name','services_category.id as category_id', 'calendar.is_empl_email', 'calendar.is_client_email')
				->get();
	...
	public static function getInfoForConfirmEmail($cartId, $nameDomain)
	{
		$calendar = self::where('domain', $nameDomain)
			...
			->leftJoin('clients', 'calendar.client_id', '=', 'clients.id')
			->leftJoin('avatars as client_avatars', 'client_avatars.user_id', '=', 'clients.user_id')
			->join('users', 'users.id', '=', 'clients.user_id')
			->select('calendar.description', 'calendar.time_from', 'calendar.time_to', 'calendar.date', 'calendar.status', 'employees.name',
				'clients.first_name', 'clients.last_name', 'clients.email', 'services.service_name', 'services.price',
				'services.duration', 'services.description as service_description', 'avatars.path', 'client_avatars.path as client_avatar', 'firmdetails.street',

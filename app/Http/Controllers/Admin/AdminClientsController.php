...
class AdminClientsController extends AdminController
{
	...
	public function store(Requests\StoreAdminClient $request, $domain)
	{
		...
		if (Client::where('email', $request->email)->first()){
			...
			$this->admin->clients()->attach($client->id);
	...
	public function clientInfo()
	{
		...
		$this->data['orders'] = $this->client->ordersClient()->where('domain', $this->firmLink)->paginate(10);
	...
	{
		return $this->client->clientProtocol()->where('admin_id', $this->admin->id)->orderBy('created_at', 'desc')->paginate(6);
	}
}

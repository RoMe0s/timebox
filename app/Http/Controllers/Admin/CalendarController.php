...
class CalendarController extends AdminController
{
	...
	{
		if (!$request->all()) {
			$result = Employee::where(['admin_id'=> $this->idAdmin, 'status' => 'active'])
							->leftJoin('avatars', 'employees.user_id', '=', 'avatars.user_id')
                    		->select('employees.id', 'avatars.path', 'employees.phone',
                            'employees.name', 'employees.email', 'employees.group', 'employees.status', 'employees.last_name')->get();
		} else {
	...
	public function addCalendar(Request $request, $domain)
	{
		...
		try {
			...
			if ($request->has('email')) {
				...
				$calendarData = [
					...
					'is_empl_email'         => $request->input('isEmplMail') ?: 0,
					'is_client_email'         => $request->input('isClientMail') ?: 0,
				];
			} else {
			...
			if ($request->action == 'create') {
				...
				$subject_empl = \Lang::get('emails.create_head_empl', [], $locale);
				...
				$subject_empl = \Lang::get('emails.edit_head_empl', [], $locale);
				...
				$subject_empl = \Lang::get('emails.del_head_empl', [], $locale);
				$data = Calendar::getInfoForConfirmEmail($cartId, $domain);
		...
		$data = $data->toArray();
		$email = $client->email;
		$email_empl = $request->input('emailEmpl');
		if ($request->action == 'create') {
			$data['status'] = 'create';
		}elseif($request->action == 'edit'){
			$data['status'] = 'edit';
		}
		$data['comment'] = $request->input('description');

		//need full path for employee avatar
		$avatar = $data['path'] ?: '/images/default_avatar.png';
		$data['path'] = public_path() . $avatar;
		$avatar_client = $data['client_avatar'] ?: '/images/default_avatar.png';
		$data['client_path'] = public_path() . $avatar_client;
		$data['locale'] = $locale;
		//send approve with subject
		if($request->input('isClientMail'))
		...
		if($request->input('isEmplMail'))
		Mail::send('emails.confirm_order_empl',
			$data,
			function ($message)
			use ($email_empl, $domain, $subject_empl, $data) {
				$message->from($domain . '@timebox24.com', $domain);
				$message->to($email_empl)->subject($subject_empl);
			});		

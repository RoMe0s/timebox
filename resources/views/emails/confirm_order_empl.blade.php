...
<div style="width: 629px; background: #ffffff; border: 1px solid #e9e9e9; border-top: 0px solid; padding: 15px 35px;">
  @if($status == 'create')
    <h2>@lang('emails.approve_employer', [], $locale)</h2>
  @elseif($status == 'edit')
    <h2>@lang('emails.edit_order_employer', [], $locale)</h2>
  @else
    <h2>@lang('emails.delete_order_employer', [], $locale)</h2>
  @endif

  <div style="margin-bottom: 30px;">
      <p style="margin: 0px;">@lang('emails.hello', [], $locale), <strong>{{$name}} !</strong></p>
      @if($status == 'create')
        <p style="margin: 0px;">@lang('emails.message_approve_create_order_employer', [], $locale)</p>
      @elseif($status == 'edit')
        <p style="margin: 0px;">@lang('emails.message_approve_edit_order_employer', [], $locale)</p>
      @else
        <p style="margin: 0px;">@lang('emails.message_approve_delete_order_employer', [], $locale)</p>
      @endif
  </div>

  <div style="height: 110px; width: 110px; overflow: hidden; float: left;">
    <img src="{{ $message->embed($client_path) }}" style="max-width: 110px;">
  </div>
  ...
  <div style="display: inline-block; vertical-align: top; padding: 0px 0 0 16px; width: 75%;">
    ...
    <p style="margin: 0 0 4px 0;"><strong>@lang('emails.client_title', [], $locale):</strong>  {{$first_name}} {{$last_name}}.</p>
    ...
    @if($status == 'deleted')
      <p style="margin: 0 0 4px 0;"><strong>Status:</strong>  @lang('emails.status_appl', [], $locale)</p>
    @endif
    @if($comment)
      <p style="margin: 0 0 4px 0;"><strong>@lang('emails.comment_appl_title', [], $locale):</strong>  {{$comment}} </p>
    @endif

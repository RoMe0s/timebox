...
<div style="padding: 20px;">
	...
	<span style="font-size: 11px; padding: 0 4px;">{{$admin_bank_details->legal_firm_name}}</span><br>
	<span style="font-size: 11px; padding: 0 4px;">{{$admin_details->firstname}} {{$admin_details->lastname}}</span><br>
	<span style="font-size: 11px; padding: 0 4px;">{{$admin_bank_details->legal_street}}</span><br>
	<span style="font-size: 11px; padding: 0 4px;">{{$admin_bank_details->city->name}}</span><br>
	<span style="font-size: 11px; padding: 0 4px;">{{$admin_bank_details->country->name}}</span>
	</p>
	...
	<table>
		...
		@if($order_details->employee_count)
			<tr>
				<td class="number">{{$i}}</td>
				<td class="number">{{number_format($order_details->employee_count, 2)}}</td>
				<td>Stück</td>
				<td>Mitarbeiter</br>
				    - {{$order_details->name}}<br>
				    vom {{$order_details->created_at}}
	...
	@if($admin_bank_details->agreement)
		<p style="margin: 8px 0; font-size: 14px;">
			Der Gesamtbetrag ist sofort fällig und wird von Ihrem Konto mit der IBAN XXX{{substr($admin_bank_details->iban, 3)}}
			(Inhaber: {{$admin_bank_details->account_owner}}) bei der {{$admin_bank_details->bank_name}},
			BIC {{$admin_bank_details->bic}} unter Verwendung der Gläubiger ID
			{{$bank_details->iban}} eingezogen.
		</p>
	@else

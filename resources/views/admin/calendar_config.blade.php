...
<section class="admin-settings director-main">
    <h1 class="director-content__heading heading heading__h1">{{trans('calendar.title_config')}}</h1>
    ...
    <template id="settings-kalendar-template">
        ...
        >
            ...
            <strong>{{trans('calendar.success_change')}}</strong>
            <p>{{trans('calendar.success_text')}}</p>
        </alert>
        <div class="block">
            <form class="settings-calendar" @submit.prevent="setSettings" action="/office/kalendar_config/edit" method="post">
                {{csrf_field()}}
                <h3 class="settings-calendar__heading">{{trans('calendar.reminder_min')}}</h3>
                ...
                <h3 class="settings-calendar__heading">{{trans('calendar.reminder_sms')}}</h3>
